...
def next_generation(world):
    nbrs_count = sum(np.roll(np.roll(world, i, 0), j, 1)
                 for i in (-1, 0, 1) for j in (-1, 0, 1)
                 if (i != 0 or j != 0))

    world = (nbrs_count == 3) | ((world == 1) & (nbrs_count == 2)).astype('int')

    # new_world = (nbrs_count == 3) | ((world == 1) & (nbrs_count == 2)).astype('int')
    # cell_ch = np.abs(new_world - world)
    # [ch_axs] = np.where(cell_ch==1)
...
def move():
    for event in pygame.event.get():
        if event.type == KEYDOWN and event.key == K_SPACE:  # 空格键暂停
            return 'Stop'
        if event.type == KEYDOWN and event.key == K_r:      # r键重置
            return 'Reset'
    ...
    # 更新时间 0.1
    if time.clock() - pygame.clock_start > 0.1:
        pygame.world = next_generation(pygame.world)   # 进化到下一代
        draw()              # 绘制
